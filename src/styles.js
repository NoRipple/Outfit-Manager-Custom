...
export function injectStyles() {
    ...
    s.id = 'om-style-v4';
        s.textContent = [
            ...
            /* ══ 顶栏 ══ */
            '.om-head{display:flex;align-items:center;gap:8px;padding:12px 15px;padding-top:max(12px, env(safe-area-inset-top, 12px));flex-shrink:0;',
            ...
            /* ══ 分类栏容器（catbar + 展开按钮）══ */
            '.om-catbar-wrap{display:flex;align-items:center;flex-shrink:0;border-bottom:1px solid rgba(127,127,127,.08);}',
            '.om-catbar-wrap .om-catbar{flex:1;border-bottom:none;}',
            '.om-acc-toggle{background:none;border:none;border-left:1px solid rgba(127,127,127,.1);',
            ...
            '.om-acc-catbar::-webkit-scrollbar{display:none;}',
            '.om-acc-catbar-label{font-size:.7em;opacity:.45;flex-shrink:0;margin-right:2px;}',
            ...
            /* 底部渐变文字遮罩 */
            /* 触屏：点击过的卡片菜单常显 */
            '@media (hover:none){.om-card-menu{opacity:.92 !important;}}',
            ...
            '.om-card-fav-mark{position:absolute;top:7px;left:7px;color:var(--SmartThemeQuoteColor,#7c6daf);',
            'font-size:.8em;line-height:1;z-index:3;pointer-events:none;opacity:.95;',
            'filter:drop-shadow(0 1px 2px rgba(0,0,0,.45));}',
            '.om-card-imgcount{position:absolute;bottom:6px;left:6px;background:rgba(0,0,0,.62);color:#fff;',
            'font-size:.66em;font-weight:600;padding:2px 7px;border-radius:10px;line-height:1.2;z-index:3;',
            'pointer-events:none;letter-spacing:.4px;}',
            '.om-card.batch-sel{border:2px solid var(--SmartThemeQuoteColor,#7c6daf);}',
            '.om-card.kit-selected{border-color:var(--SmartThemeQuoteColor,#7c6daf);',
            'box-shadow:0 0 0 1px var(--SmartThemeQuoteColor,#7c6daf),0 4px 16px rgba(0,0,0,.2);}',
            '.om-kit-badge{z-index:2;}',

            /* 卡片菜单按钮 - 右下角，不与对号冲突 */
            '.om-card-menu{position:absolute;bottom:-3px;right:-1px;',
            'width:34px;height:32px;border:none;border-radius:8px;',
            'background:transparent;color:var(--SmartThemeQuoteColor,#7c6daf);cursor:pointer;',
            'display:flex;align-items:center;justify-content:center;font-size:1.05em;font-weight:900;line-height:1;overflow:hidden;',
            'opacity:0;transition:opacity .18s,transform .18s;z-index:3;pointer-events:auto;',
            'filter:drop-shadow(0 1px 2px rgba(0,0,0,.55));}',
            '.om-card:hover .om-card-menu,.om-card:active .om-card-menu{opacity:1;}',
            '.om-card-menu:hover{background:transparent;opacity:1;transform:scale(1.05);}',
            ...
            /* ══ 底栏 ══ */
            '.om-bottombar{display:flex !important;align-items:center;gap:6px;padding:10px 14px;padding-bottom:max(10px, env(safe-area-inset-bottom, 10px));flex-shrink:0;',
            ...
            '.om-detail-loose-row{display:flex;gap:6px;overflow-x:auto;flex-wrap:nowrap;',
            '-webkit-overflow-scrolling:touch;scrollbar-width:none;padding:4px 0;}',
            '.om-detail-loose-row::-webkit-scrollbar{display:none;}',
            '.om-detail-divider{height:1px;background:rgba(127,127,127,.15);margin:6px 0;}',
            ...
            '.om-subject-tag{cursor:pointer;flex-shrink:0;white-space:nowrap;}',
            '.om-subject-tag.focus{box-shadow:0 0 0 2px rgba(255,255,255,.24) inset;}',
            '.om-kit-rows{display:flex;flex-direction:column;gap:8px;}',
            '.om-kit-row{display:flex;align-items:center;gap:6px;flex-wrap:nowrap;min-width:0;}',
            '.om-kit-row.focus{padding:5px;border-radius:10px;background:rgba(127,127,127,.08);}',
            '.om-kit-accs{display:flex;align-items:center;gap:5px;flex-wrap:wrap;min-width:0;}',
            '.om-kit-accs-scroll{display:flex;align-items:center;gap:5px;overflow-x:auto;flex:1;min-width:0;',
            '-webkit-overflow-scrolling:touch;scrollbar-width:none;}',
            '.om-kit-accs-scroll::-webkit-scrollbar{display:none;}',
            '.om-kit-acc-tag{border:1px solid rgba(127,127,127,.18);border-radius:13px;',
            'background:rgba(127,127,127,.08);color:inherit;padding:4px 9px;font-size:.75em;',
            'font-family:inherit;cursor:pointer;line-height:1.2;white-space:nowrap;flex-shrink:0;}',
            '.om-kit-acc-tag:hover{border-color:var(--SmartThemeQuoteColor,#7c6daf);color:var(--SmartThemeQuoteColor,#7c6daf);}',
            '.om-kit-acc-tag.disabled{opacity:.45;text-decoration:line-through;background:rgba(127,127,127,.04);}',
            '.om-kit-save{border:none;border-radius:8px;background:var(--SmartThemeQuoteColor,#7c6daf);',
            'color:#fff;padding:5px 10px;font-size:.75em;font-weight:700;font-family:inherit;cursor:pointer;flex-shrink:0;}',
            ...
            '.om-showcase{display:flex;gap:8px;align-items:stretch;width:100%;height:100%;min-height:120px;overflow:hidden;}',
            '.om-showcase-main{flex-shrink:0;width:35%;max-width:140px;min-width:92px;',
            'border:1px dashed rgba(127,127,127,.2);border-radius:10px;overflow:hidden;box-sizing:border-box;background:rgba(127,127,127,.04);}',
            '.om-imgarea .om-showcase-main-img{width:100%;height:100%;object-fit:cover;display:block;}',
            '.om-showcase-ph{height:100%;justify-content:center;}',
            '.om-showcase-accs{display:flex;gap:6px;overflow-x:auto;flex:1;min-width:0;height:100%;',
            '-webkit-overflow-scrolling:touch;scrollbar-width:none;padding:2px 0;box-sizing:border-box;}',
            '.om-showcase-accs::-webkit-scrollbar{display:none;}',
            '.om-imgarea .om-showcase-acc-img{width:auto;height:100%;max-height:150px;border-radius:8px;',
            'border:1px solid rgba(127,127,127,.15);object-fit:cover;flex-shrink:0;box-sizing:border-box;}',
            '.om-showcase-acc-ph{display:flex;align-items:center;justify-content:center;min-width:70px;height:100%;',
            'border-radius:8px;border:1px dashed rgba(127,127,127,.2);background:rgba(127,127,127,.06);font-size:.72em;opacity:.62;',
            'flex-shrink:0;padding:0 8px;box-sizing:border-box;text-align:center;word-break:break-all;}',
            '.om-img-actions{display:flex;gap:7px;margin-top:7px;flex-wrap:wrap;}',
            /* 多图缩略图 */
            '.om-imgarea.om-imgs{height:auto;min-height:96px;display:block;}',
            '.om-imgs-grid{display:flex;flex-wrap:wrap;gap:8px;align-items:flex-start;width:100%;}',
            '.om-img-thumb{position:relative;width:76px;height:76px;border-radius:9px;overflow:hidden;',
            'border:1px solid rgba(127,127,127,.2);background:rgba(127,127,127,.08);flex-shrink:0;}',
            '.om-img-thumb.primary{border-color:var(--SmartThemeQuoteColor,#7c6daf);box-shadow:0 0 0 1px var(--SmartThemeQuoteColor,#7c6daf);}',
            '.om-img-thumb img{width:100%;height:100%;object-fit:cover;display:block;}',
            '.om-img-thumb-no{position:absolute;bottom:3px;left:3px;background:rgba(0,0,0,.62);color:#fff;',
            'font-size:.62em;font-weight:600;padding:1px 6px;border-radius:8px;line-height:1.3;pointer-events:none;}',
            '.om-img-thumb-del{position:absolute;top:2px;right:2px;width:18px;height:18px;border:none;border-radius:50%;',
            'background:rgba(0,0,0,.6);color:#fff;font-size:.6em;display:flex;align-items:center;justify-content:center;',
            'cursor:pointer;padding:0;line-height:1;}',
            '.om-img-thumb-cover{position:absolute;top:2px;left:2px;width:18px;height:18px;border:none;border-radius:50%;',
            'background:rgba(0,0,0,.6);color:var(--SmartThemeQuoteColor,#7c6daf);font-size:.58em;display:flex;align-items:center;',
            'justify-content:center;cursor:pointer;padding:0;line-height:1;}',
            '.om-img-thumb.om-img-add{display:flex;flex-direction:column;align-items:center;justify-content:center;gap:3px;',
            'border-style:dashed;color:rgba(127,127,127,.6);font-size:.72em;cursor:pointer;background:transparent;}',
            '.om-img-thumb.om-img-add i{font-size:1.2em;}',
            '.om-showcase-wrap{height:150px;}',
            '.om-showcase-wrap .om-showcase{height:100%;}',
            ...
            '.om-kit-empty{padding:9px 10px;border-radius:8px;background:rgba(127,127,127,.06);',
            'font-size:.82em;opacity:.55;}',
            '.om-kit-empty.small{padding:0;background:none;font-size:.76em;}',
            '.om-edit-kit-row{display:flex;align-items:flex-start;gap:8px;padding:9px 10px;',
            'border:1px solid rgba(127,127,127,.12);border-radius:8px;background:rgba(127,127,127,.05);margin-bottom:7px;}',
            '.om-edit-kit-row.active{border-color:var(--SmartThemeQuoteColor,#7c6daf);background:rgba(124,109,175,.08);}',
            '.om-edit-kit-main{flex:1;min-width:0;}',
            '.om-edit-kit-name{display:flex;align-items:center;gap:6px;font-size:.88em;font-weight:700;}',
            '.om-edit-kit-active{font-size:.72em;font-weight:600;padding:1px 5px;border-radius:8px;',
            'background:var(--SmartThemeQuoteColor,#7c6daf);color:#fff;}',
            '.om-edit-kit-count{font-size:.74em;opacity:.5;margin-top:2px;}',
            '.om-edit-kit-accs{display:flex;gap:5px;flex-wrap:wrap;margin-top:7px;}',
            '.om-edit-kit-acc{display:inline-flex;align-items:center;gap:5px;border:1px solid rgba(127,127,127,.16);',
            'border-radius:13px;background:rgba(127,127,127,.07);color:inherit;padding:3px 7px;font-size:.74em;',
            'font-family:inherit;cursor:pointer;}',
            '.om-edit-kit-acc:hover{border-color:#e57373;color:#e57373;}',
            '.om-edit-kit-actions{display:flex;gap:5px;flex-shrink:0;}',
            '.om-kit-deactivate{align-self:flex-start;font-size:.78em;margin:0 0 8px 0;}',
            '.om-btn-sm.danger{color:#e57373;border-color:rgba(229,115,115,.35);}',
            '.om-btn-sm.danger:hover{background:#e57373;color:#fff;}',
            /* 场景标签建议 */
            ...
            /* 初始化失败安全恢复窗口 */
            '.om-recovery-overlay{position:fixed;inset:0;z-index:2147483647;background:rgba(0,0,0,.62);',
            'display:flex;align-items:center;justify-content:center;padding:18px;box-sizing:border-box;',
            'color:var(--SmartThemeBodyColor,#eee);animation:om-fadein .18s ease;}',
            '.om-recovery-card{position:relative;width:min(520px,100%);max-height:min(760px,92dvh);overflow:auto;',
            'background:var(--SmartThemeBackgroundColor,#1d1d22);border:1px solid rgba(127,127,127,.24);',
            'border-radius:16px;padding:24px 22px 20px;box-sizing:border-box;box-shadow:0 16px 50px rgba(0,0,0,.5);}',
            '.om-recovery-close{position:absolute;right:10px;top:10px;width:34px;height:34px;border:none;border-radius:50%;',
            'background:transparent;color:inherit;opacity:.55;cursor:pointer;font-size:1.05em;}',
            '.om-recovery-close:hover{opacity:1;background:rgba(127,127,127,.12);}',
            '.om-recovery-icon{width:42px;height:42px;border-radius:50%;display:flex;align-items:center;justify-content:center;',
            'background:rgba(255,152,0,.14);color:#ffb74d;font-size:1.15em;margin-bottom:12px;}',
            '.om-recovery-title{font-size:1.12em;font-weight:750;padding-right:34px;margin-bottom:7px;}',
            '.om-recovery-safe{font-size:.86em;line-height:1.6;color:#81c784;margin-bottom:12px;}',
            '.om-recovery-status{padding:10px 12px;border-radius:9px;background:rgba(127,127,127,.08);',
            'font-size:.86em;line-height:1.55;margin-bottom:10px;}',
            '.om-recovery-issues{display:flex;flex-direction:column;gap:7px;margin:10px 0;}',
            '.om-recovery-issue{display:flex;align-items:flex-start;gap:8px;padding:9px 11px;border-radius:9px;',
            'background:rgba(229,115,115,.09);border:1px solid rgba(229,115,115,.2);font-size:.83em;line-height:1.5;}',
            '.om-recovery-issue code{flex-shrink:0;color:#ef9a9a;font-size:.8em;padding-top:1px;}',
            '.om-recovery-snapshot{font-size:.84em;padding:9px 11px;border-radius:9px;background:rgba(129,199,132,.1);',
            'border:1px solid rgba(129,199,132,.18);margin:10px 0;}',
            '.om-recovery-snapshot.muted{opacity:.62;background:rgba(127,127,127,.06);border-color:rgba(127,127,127,.12);}',
            '.om-recovery-details{font-size:.8em;line-height:1.65;margin:10px 0 15px;opacity:.78;}',
            '.om-recovery-details summary{cursor:pointer;margin-bottom:5px;}',
            '.om-recovery-actions{display:flex;gap:8px;justify-content:flex-end;flex-wrap:wrap;}',
            '.om-recovery-actions .om-btn{min-height:38px;}',
            '.om-recovery-actions .om-btn:disabled{opacity:.45;cursor:not-allowed;}',
            '.om-recovery-busy{font-size:.82em;text-align:center;margin-top:12px;opacity:.72;}',
            '@media(max-width:520px){.om-recovery-overlay{align-items:flex-end;padding:10px;}',
            '.om-recovery-card{border-radius:16px 16px 10px 10px;padding:22px 16px 16px;max-height:90dvh;}',
            '.om-recovery-actions{display:grid;grid-template-columns:1fr;}',
            '.om-recovery-actions .om-btn{width:100%;justify-content:center;}}',
            /* 全屏 lightbox */
